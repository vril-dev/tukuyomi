...
- `conf/scheduled-tasks.json`: scheduled tasks を使う場合の初期 seed/import file

その後、本番起動前に `make db-migrate` を実行し、新規 DB へ file material を
取り込む場合は `make db-import` も実行します。
...
- DB `config_blobs.app_config`: global runtime / listener / admin / storage policy / path 設定
- DB `config_blobs.proxy_rules`: live の proxy transport / routing 設定
- `data/conf/proxy.json`: proxy rules の seed/import/export material
- DB `config_blobs.proxy_rules.backend_pools[]`: named upstream member から作る route 単位の balancing group
...
- `conf/scheduled-tasks.json`: scheduled task の seed/import/export material
