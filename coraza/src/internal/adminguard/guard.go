...
var (
	...
	currentAccess *accessControl
	currentRate   *rateLimiter
)
...
func AccessMiddleware(endpointKind string) gin.HandlerFunc {
	return func(c *gin.Context) {
		access := currentAccessControl()
...
func CheckUIAccess(r *http.Request) bool {
	access := currentAccessControl()
	return access == nil || access.allowsEndpoint(r, endpointUI)
}

func EvaluateUIRateLimit(r *http.Request) RateLimitDecision {
	limiter := currentRateLimiter()
	if limiter == nil {
...
func StatsSnapshot() RateLimitStats {
	limiter := currentRateLimiter()
	if limiter == nil {
...
func TrustedForwardedHeaders(r *http.Request) bool {
	...
	return isTrustedIP(trustedCIDRsSnapshot(), sourceIP)
}

func currentAccessControl() *accessControl {
	guardMu.RLock()
...
func (a *accessControl) Update() error {
	...
	if len(cidrs) == 0 {
		cidrs = append([]string(nil), defaultTrustedCIDRs...)
	}
...
func (a *accessControl) allowsEndpoint(r *http.Request, endpointKind string) bool {
	if a == nil {
		return true
	}
	a.mu.RLock()
	...
	clientIP, ok := resolveClientIP(r, trustForwardedFor, trustedCIDRs)
	if !ok {
		return false
	}
	if isTrustedIP(trustedCIDRs, clientIP) {
		return true
	}
	...
	case "api_only_external":
		return endpointKind == endpointAPI
	default:
		return false
	}
}
...
func (r *rateLimiter) Evaluate(req *http.Request) RateLimitDecision {
	if r == nil {
		return RateLimitDecision{Allowed: true}
	}
	r.mu.Lock()
...
func (r *rateLimiter) pruneLocked(cutoff time.Time) {
...
func isTrustedIP(prefixes []netip.Prefix, ip netip.Addr) bool {
...
func trustedCIDRsSnapshot() []netip.Prefix {
	access := currentAccessControl()
	if access == nil {
		return nil
	}
...
func resolveClientIP(r *http.Request, trustForwarded bool, trustedCIDRs []netip.Prefix) (netip.Addr, bool) {
	if r == nil {
	...
	if trustForwarded && isTrustedIP(trustedCIDRs, sourceIP) {
