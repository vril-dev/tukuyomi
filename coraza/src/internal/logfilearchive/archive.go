...
func (a *Archive) Append(obj map[string]any, path string) error {
...
func (a *Archive) AppendEncoded(raw []byte, path string) error {
	return a.AppendEncodedBatch([][]byte{raw}, path)
}

func (a *Archive) AppendEncodedBatch(raws [][]byte, path string) error {
	if a == nil {
		return appendRawLines(raws, path)
	}
	if len(raws) == 0 {
		return nil
	}
	...
	if config.FileRotateBytes > 0 {
		if info, err := os.Stat(path); err == nil && info.Size() > 0 && info.Size()+int64(lineLen) > config.FileRotateBytes {
			if err := RotateGzip(path); err != nil {
				return err
			}
			rotated = true
		}
	...
	if err := PruneManaged(path, now); err != nil {
		return err
	}
...
func appendRawLines(raws [][]byte, path string) error {
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		return err
	}
	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
...
func (a *Archive) pruneDueLocked(path string, now time.Time, rotated bool) bool {
	...
	return last.IsZero() || !now.Before(last.Add(PruneInterval))
}

func (a *Archive) markPrunedLocked(path string, now time.Time) {
	if a.lastPrune == nil {
...
func RotateGzip(path string) error {
...
func PruneManaged(path string, now time.Time) error {
	files, err := ListManaged(path)
	if err != nil {
		return err
	}
...
func ListManaged(path string) ([]File, error) {
