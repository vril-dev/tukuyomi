...
func TestArchiveAppendThrottlesUnrotatedPrune(t *testing.T) {
	restore := saveConfigForTest()
	defer restore()
	...
	archive.mu.Lock()
	archive.lastPrune[path] = time.Now().Add(-PruneInterval - time.Second)
	archive.mu.Unlock()
...
func TestArchiveAppendEncodedWritesRawJSONLine(t *testing.T) {
	restore := saveConfigForTest()
	defer restore()
	...
	if err := (&Archive{}).AppendEncoded(raw, path); err != nil {
...
func TestArchiveAppendEncodedBatchWritesRawJSONLines(t *testing.T) {
	restore := saveConfigForTest()
	defer restore()
	...
	if err := (&Archive{}).AppendEncodedBatch(raws, path); err != nil {
