...
- `INSTALL_ROLE=center-protected` は `tukuyomi.service` と `tukuyomi-center.service` の両方を導入します。Center は loopback で待ち受け、Gateway seed は `/center-ui`、operator 向けの `/center-manage-api`、Gateway/device 向けの `/center-api` path を `http://127.0.0.1:9092` へ転送します。remote Gateway は `/center-api/v1` 配下をポーリングします。Gateway の IoT / Edge device authentication を有効化し、対応する Center 承認もローカルで bootstrap します。初回ホスト設定で TLS / Let's Encrypt、リスナーポート、Gateway front 設定を終えられるよう、この role だけは Gateway 管理 UI を `admin.external_mode=full_external` で外部から到達可能にします。初期設定後は Gateway Settings で `admin.external_mode` を `api_only_external` または `deny_external` に戻してください。初回から厳しく閉じたい場合は `INSTALL_CENTER_PROTECTED_GATEWAY_ADMIN_EXTERNAL_MODE=api_only_external` を指定します。この role は同一ホスト上の Gateway も含むため、スケジュールタスク用タイマーも既定で導入します
- `INSTALL_CENTER_API_BASE_PATH` は Center UI が使う管理 API path です。既定値は `/center-manage-api` です。`INSTALL_CENTER_GATEWAY_API_BASE_PATH` は Gateway/device 向けに公開する API path で、既定値は `/center-api` です
- `center-protected` では、`/center-ui` と `/center-manage-api` の管理用 route を Proxy Rules の `access.allow_cidrs` で制限します。remote Gateway がポーリングする `/center-api/v1` の Gateway/device API route は開けたままにします
...
ACME は、`TLS` 画面で TLS binding の `mode=acme` を選択して設定します。
`production` ／ `staging` は Let's Encrypt の本番 CA ／ステージング CA の選択で、アカウントメールは任意です。
HTTP-01 チャレンジを使うため、DNS 名がこのホストを指し、外部から TCP/80 と TCP/443 に到達できる必要があります。

VPS や bare metal で初期設定ポートから `:80` / `:443` へ移行する場合は、次の順で進めます。

1. `Settings` の `Listener & Network` で、`Public listener rows` に `:80` の HTTP listener と `:443` の HTTPS listener を用意します。初期設定中は `:80` の `HTTP behavior` を `serve` のままにしておくと、管理 UI に戻る経路を残しやすくなります。
2. 同じ画面で `Enable built-in TLS on the public listener` を有効にし、`Save config only` を押します。
3. `systemctl restart tukuyomi` で Gateway を再起動します。
4. `TLS` 画面で TLS binding を追加し、`TLS mode=acme`、`Hosts=<DNS 名>` を設定します。最初は `staging` で HTTP-01 チャレンジと HTTPS 到達性を確認します。
5. `https://<DNS 名>/tukuyomi-ui/` にアクセスできることを確認してから、`ACME environment` を `production` に変更して `Apply` します。

`https://<IP アドレス>/...` でアクセスすると、証明書のホスト名と一致しないため警告が出ます。必ず TLS binding の `Hosts` に設定した DNS 名で確認してください。`staging` 証明書で確認したブラウザーは警告状態を保持することがあるため、`production` へ切り替えた後は新しいタブやシークレットウィンドウでも確認します。
...
`server.public_listeners` を使う場合も、有効な低位ポートリスナーは同じケーパビリティ要件を持ちます。
