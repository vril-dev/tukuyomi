...
- `center-protected` では、`/center-ui` など operator 向けの Center route を Proxy Rules の `access.allow_cidrs` で制限します。remote Gateway がその path からポーリングする必要がある場合は、Gateway/device API route は開けたままにします
