...
HTTP-01 チャレンジを使うため、`server.tls.redirect_http=true` と `server.tls.http_redirect_addr=:80`、または同等のポート 80 転送を用意してください。新規の VPS ／ bare metal 配備では、既存の初期設定用リスナーを残したまま `:443` を追加できる `server.public_listeners` を優先してください。
...
この構成では `:9090` を残したまま、`:443` と `:80` の動作を確認できます。外部から HTTPS で到達できることを確認してから、不要であれば `setup` 行を削除してください。
