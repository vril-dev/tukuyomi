...
- `seeds/conf/config-bundle.json` は空 DB 向けの同梱シードとしてマウントまたはイメージにベイクします。`conf/proxy.json` や各種ポリシーファイルなどの設定済みファイルがある場合はそちらが優先されます
...
WAF log archive は、同じ `persistent_storage` バックエンド内の
`log-archives/waf/` 名前空間を使います。

プロキシエンジン設定は、現在 `tukuyomi_proxy` 固定です。
