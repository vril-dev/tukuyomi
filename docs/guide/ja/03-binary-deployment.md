...
配備手順を扱います。想定環境はオンプレ Linux サーバー、VPS、各種クラウド VM
（EC2 / GCE / Azure VM など）です。コンテナ配備は次の第4章で扱います。

第2章の preview と違い、本章で扱うのは **本番運用に耐える形での配備** です。
service ユーザーの作成、`/opt/tukuyomi` 以下の実行レイアウト、systemd ユニット、
...
必要なものは、`make`、Go toolchain 1.26.4 以上、そして Gateway / Center UI
...
インストール実行ユーザーが Docker API にアクセスできる状態にしてください。

Ubuntu VPS では、たとえば次のように準備します。
...
Go 公式 tarball から 1.26.4 以上を導入します。
...
再現性のある release artifact を作るときは次を使います。
...
明示的に上書きする場合だけ `VERSION=vX.Y.Z` を指定します。
...
初回ログイン用の owner を明示したい場合は、次のようにインストール時に渡します。
省略した場合は、インストーラーがランダムな初期パスワードを生成し、インストールログの最後に 1 回だけ表示します。
...
- `PREFIX` がインストール実行ユーザーの home 配下にある場合、`INSTALL_CREATE_USER=auto`
  はそのユーザーを runtime user として使い、`useradd` を実行しません。
...
front の初期 seed には `/center-ui`、operator 向けの `/center-manage-api`、
Gateway/device 向けの `/center-api` path を `http://127.0.0.1:9092` へ転送する
path-scoped route を入れます。remote Gateway は `/center-api/v1` 配下をポーリングします。導入時には
...
初回ホスト設定のため、`center-protected` は Gateway 管理 UI を
`admin.external_mode=full_external` で外部から到達可能にします。これにより、
TLS / Let's Encrypt、リスナーポート、Gateway front 設定をブラウザーから完了
できます。設定が終わったら、Gateway Settings で `admin.external_mode` を
`api_only_external` または `deny_external` に戻し、Gateway 管理 UI の露出を
閉じてください。初回起動から閉じたい場合は
`INSTALL_CENTER_PROTECTED_GATEWAY_ADMIN_EXTERNAL_MODE=api_only_external` を指定します。

`INSTALL_CENTER_API_BASE_PATH` は Center UI が使う管理 API path で、既定値は
`/center-manage-api` です。`INSTALL_CENTER_GATEWAY_API_BASE_PATH` は
Gateway/device 向けに公開する API path で、既定値は `/center-api` です。

center-protected host では、operator が触る Center path は Gateway の route 単位で
制限します。たとえば `/center-ui` と `/center-manage-api` の管理用 route に
`access.allow_cidrs` を設定し、remote Gateway がポーリングする `/center-api/v1`
...
- MySQL / PostgreSQL の空 DB を初期投入する場合は、`INSTALL_DB_SEED=always` を
  明示的に指定してください。
- migration 後、host install は導入対象 role ごとの DB に初期 owner bootstrap を実行します。`TUKUYOMI_ADMIN_BOOTSTRAP_PASSWORD` が未指定、または開発用プレースホルダーのままの場合は、ランダムな初期パスワードを生成してインストールログの最後に 1 回だけ表示します。既存の管理ユーザーはリセットしません。
...
runtime authority ではなくなります（DB を正(SoT)として保持します）。
...
account email は任意です。HTTP-01 challenge を使うため、DNS 名がこのホストを
指し、外部から TCP/80 と TCP/443 に到達できる必要があります。

VPS / bare metal で初期設定ポートから `:80` / `:443` へ移行する場合は、
ブラウザーで次の順に進めます。

1. `Settings` の `Listener & Network` で、`Public listener rows` に `:80`
   の HTTP listener と `:443` の HTTPS listener を用意します。初期設定中は
   `:80` の `HTTP behavior` を `serve` のままにしておくと、TLS 設定前でも
   管理 UI へ戻る経路を残せます。
2. 同じ画面で `Enable built-in TLS on the public listener` を有効にし、
   `Save config only` を押します。
3. `systemctl restart tukuyomi` で Gateway を再起動します。
4. `TLS` 画面で TLS binding を追加し、`TLS mode=acme`、`Hosts=<DNS 名>` を
   設定します。最初は `staging` で HTTP-01 challenge と HTTPS 到達性を
   確認します。
5. `https://<DNS 名>/tukuyomi-ui/` にアクセスできることを確認してから、
   `ACME environment` を `production` に変更して `Apply` します。

`https://<IP アドレス>/...` でアクセスすると、証明書のホスト名と一致しないため
警告が出ます。必ず TLS binding の `Hosts` に設定した DNS 名で確認してください。
...
- 本番展開前には、実 workload で benchmark してください（第18章）。
...
- `server.public_listeners` を設定した場合は、その行が単一の public listener
  の代わりに使われます。`:9090` を残したまま `:80` の HTTP 行と `:443` の
  HTTPS 行を追加する段階移行に使います。
...
- built-in TLS / HTTP redirect / HTTP/3 は、この slice では public listener
  専用です。
- 明示的な `server.public_listeners` を使う場合、ACME 用に `:80` の HTTP 行を
  残します。HTTPS 確認後に HTTP から HTTPS へ redirect したい場合は、
...
- `admin.session_secret` は managed app config に置き、ブラウザーに露出させない
  ように扱います。
- `TUKUYOMI_ADMIN_BOOTSTRAP_USERNAME` / `TUKUYOMI_ADMIN_BOOTSTRAP_PASSWORD` は、
  admin user table が空の **初回 owner bootstrap だけ** に使います。
- `/auth/login` の POST が HTTP 401 を返す場合は、入力した認証情報が既存の
  管理ユーザーと一致していません。HTTP でアクセスしていること自体が 401 の
  原因ではありません。ただし本番公開前には HTTPS、または前段の TLS 終端を必ず
  用意してください。
- ブラウザー operator は username / password で sign in し、same-origin の
...
- `tukuyomi` の既定 posture は `admin.external_mode=api_only_external` です。
  ...
  例外として `INSTALL_ROLE=center-protected` は初回ホスト設定のため
  `full_external` で開始します。TLS とリスナー設定が終わったら Gateway
  Settings で再度絞ってください。
