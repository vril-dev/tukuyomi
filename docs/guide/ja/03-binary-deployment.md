...
省略した場合は、インストーラーがランダムな初期パスワードを生成して 1 回だけ表示します。
...
- MySQL / PostgreSQL の空 DB を初期投入する場合は、`INSTALL_DB_SEED=always` を
  明示的に指定してください。
- migration 後、host install は導入対象 role ごとの DB に初期 owner bootstrap を実行します。`TUKUYOMI_ADMIN_BOOTSTRAP_PASSWORD` が未指定、または開発用プレースホルダーのままの場合は、ランダムな初期パスワードを生成してインストールログに 1 回だけ表示します。既存の管理ユーザーはリセットしません。
