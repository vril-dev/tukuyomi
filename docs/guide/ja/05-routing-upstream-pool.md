...
Discovery と Sticky Sessions の動作、`dry-run` の使い方までを見ていきます。
...
`Upstreams` の各行には専用の `Probe` ボタンがあり、**画面全体に対して
...
`proxy_route` は route classification 後・final target selection 前に
出るため、selected upstream 系の field は持たない、という点を改めて
意識してください。
