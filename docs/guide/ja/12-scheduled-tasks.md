...
## 12.2.1　組み込み archive task

`make install` が scheduled tasks 有効の Gateway role を配置するとき、次の
組み込み task を bootstrap します。
...
これは `03:17 UTC` に 1 日 1 回実行され、期限切れの WAF / access event を
hot DB storage から `persistent_storage` へ移します。bootstrap は一度だけ
です。operator が後でこの task を変更、無効化、または削除した場合、以後の
install はその判断を維持します。
