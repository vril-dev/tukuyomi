...
した。本章では、**public listener 経路で TLS と HTTP/3 をどう扱うか**
...
- `server.public_listeners` を使うと、`:9090` を残したまま `:443` と
  `:80` redirect を追加するような段階移行ができます。この mode では、
  redirect は `http_behavior=redirect` の HTTP listener 行で表現します。
...
![TLS 画面](../../images/ui-samples/16-sites.png)

ACME を使う場合の前提:

- HTTP-01 challenge を使うため、**port 80 を `server.tls.http_redirect_addr`
  に到達** させる必要がある。`server.public_listeners` を使う場合は、
  `http_behavior=redirect` の HTTP listener 行から HTTPS 行へ redirect させます。
...
- ACME 自動 TLS は TLS binding で設定し、HTTP-01 のため **port 80
  forwarding が必須**。証明書 cache は
