...
> **すべての Web / VPS 環境で IoT / Edge mode を有効にする必要は
> ありません。**

通常の Web / VPS 環境は、`IoT / Edge Mode` を **OFF のまま** で
構いません。本章で扱う device authentication / Center 承認は、

- 設置先で初期登録し、Center 側で承認してから使い始める **edge ゲートウェイ**
- 多数の **IoT デバイス** を Center 承認のもとでまとめて管理する **fleet 運用**

といった、環境単位で **device identity を中央で確認したい** ケースの
...
通常の Web / VPS 環境は `IoT / Edge Mode` を OFF のままにするため、
この `503` には **当たりません**。
...
スナップショット表には、マスク済みの Gateway JSON ペイロードが上限付きで保存され、必要に応じて表示またはダウンロードできます。

この Center スナップショットは、Gateway Status の `Download config` で取得するファイルとは別物です。`Download config` の出力は seed／restore に使う `config-bundle.json` artifact です。一方、Center スナップショットはデバイス identity、revision metadata、domain ごとの `etag`／`raw` を持つ署名付き fleet status payload です。

![Center の Runtime 画面](../../images/ui-samples/27-center-runtime.png)

`Runtime` では、Gateway から報告されたプラットフォームターゲットに対応するランタイム配布物を管理します。Center は、そのターゲット向けに PHP-FPM または PSGI のランタイム配布物をビルドし、圧縮済み artifact として保存したうえで Gateway に割り当てることができます。
同じプラットフォームターゲット向けの artifact がすでに作成済みであれば、再ビルドせずに既存の artifact を選択して割り当てることもできます。

Runtime の変更は、Center UI/API の操作だけでは Gateway に即時反映されません。Center には **pending runtime requests** として登録され、Gateway が署名付きステータスのポーリングで取得したタイミングで処理されます。
Gateway は、受け取った変更要求の artifact metadata とプラットフォームターゲットを検証し、圧縮済み artifact をダウンロードしたうえで、Gateway ローカルの runtime をインストールまたは削除します。pending request は、Gateway が取得する前であれば Center から取り消せます。
runtime の削除は、Center と Gateway の両側で安全確認を行います。Center UI は最新の `Runtime inventory` をもとに危険な削除要求を無効化し、Gateway 側でもローカルの runtime ファイルを削除する直前に、Runtime App からの参照と実行中プロセスの有無を再確認します。

`Runtime App Deploy` は、`php-fpm`、`psgi`、`daemon` の Runtime Apps へ小さなアプリケーションパッケージを配信する試験的なデバイス画面です。運用者は deployment root（配備ルート）を定義し、ZIP package を Center へアップロードし、package 間の差分を確認してから、1 回限りの deploy または rollback request を作成します。必要に応じて、切り替え前後に Gateway 上でシェルスクリプトを実行できます。

この画面は Center 管理を前提にしています。package 履歴、deployment profile、差分 preview、手動 rollback の選択は Center に残り、Gateway は検証済み request を実行します。baseline adoption（初回採用）は明示的な操作です。デバイスが登録された、または Runtime App を報告しただけで、Center が Gateway のソースを黙って取得することはありません。

Gateway はアプリ単位で 1 つの deployment root を使います。
...
複数の論理ルートは、この 1 つの `current` symlink 配下に配置されます。たとえば PHP-FPM package では `public/` を `current/public` に、PSGI package では `app/` と `static/` を `current/app` と `current/static` に割り当てられます。daemon package では `app/` を `current/app` へ割り当てます。Gateway は、対象 Runtime App が期待する Center 管理パスを向いていない場合、deploy と rollback をブロックします。これにより、Center が任意の `document_root` や `app_root` を置き換えることを防ぎます。

baseline adoption または最初の Center 管理 deploy が成功すると、Gateway は対象 Runtime Apps の参照先を書き換えます。PHP-FPM では、`document_root` が元の `data/runtime-sites/<app-id>/...` 配下から、通常は `data/app-deployments/<app-id>/current/public` へ変わります。PSGI では、`app_root` や `document_root` が対応する `current/<runtime-subpath>` へ変わります。daemon app では、`app_root` が `data/app-deployments/<app-id>/current/app` へ変わり、設定済みの `persistent_paths` は `data/app-deployments/<app-id>/persistent/` 配下の安定ディレクトリへ symlink されます。切り替え後の元 `data/runtime-sites/<app-id>/...` ディレクトリは、初回採用時のソースであり、以後の反映先ではありません。以後の変更は Center へ新しい package をアップロードして配備します。ローカル管理へ戻す場合は、Runtime Apps の参照先を意図的にローカルパスへ戻してください。
...
`INSTALL_ROLE=center-protected` と同じ構成を preview する場合は、Center
...
このモードでは、Center は別プロセスのまま動き、Gateway が `/center-ui`、
operator 向けの `/center-manage-api`、Gateway/device 向けの `/center-api` path を
共有 Docker preview ネットワーク上の Center へ転送する route を初期投入します。
remote Gateway は `/center-api/v1` 配下をポーリングします。
Center は Gateway 経由の `http://localhost:9090/center-ui` から
開きます。protected preview では Gateway の IoT / Edge mode も有効化し、preview
Center DB に対する Center 承認も初期登録します。
...
DB が作成されるタイミングでのみ初期投入されます。既存の永続 Gateway preview
DB がすでにある場合は、その preview DB をリセットするか、`Proxy Rules` から
route を追加してください。

Gateway が preview コンテナ内で動いている場合、**Center URL に
`http://localhost:9092` を指定しない** でください。Gateway コンテナ内の
...
コンテナ側に **host-gateway mapping を設定** するか、到達可能な Center
アドレスを指定してください。
...
- 通常の Web / VPS 環境では **IoT / Edge Mode は OFF のまま**。
...
次の第17章では、本章で扱った Center 管理のデバイスフローの上に乗る
**Remote SSH** を扱います。Gateway のインバウンド SSH ポートを開けず、Center 経由で
短時間の保守接続を行うための運用導線です。
