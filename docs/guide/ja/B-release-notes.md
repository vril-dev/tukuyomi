...
（DB-backed runtime authority、admin 認証の刷新、`make install` の整備）
として併載しています。
...
- **Gateway に IoT / Edge mode を追加** しました。有効化すると、local
  Gateway identity が Center で承認されるまで public proxy traffic は
  ロックされます。
...
- **IoT / Edge mode はデフォルト OFF** です。Web / VPS deployment では、
...
- Gateway Options で IoT / Edge status refresh の **Center polling interval**
  を設定できます。
- Gateway は device approval state に応じて、proxy traffic が **available
  / locked のどちらかを表示** します。
...
- **Center User** で username、email、password、personal access token を
  管理できます。
- **Gateway と Center の browser session は分離** しました。同じ browser で
  両 UI に同時ログインできます。
...
- 本番用シードデータは **`seeds/conf` 配下** に配置しました。初回取り込み
  データを調整する場合は、Go コードではなく seed file を編集してください。
...
以上で、tukuyomi v1.2.0 / v1.1.0 のリリースノートと、本書の本編 + 付録
A / B をすべて収めました。

本書の更新は **上流リポジトリ（`/home/ky491/git/vril/tukuyomi`）の
`README.ja.md` および `docs/**/*.ja.md`** を一次情報として追従します。
記述に齟齬を見つけた場合は、上流ドキュメントを優先してください。
