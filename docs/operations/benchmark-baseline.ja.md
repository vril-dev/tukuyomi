...
| `BENCH_PROXY_ENGINE` | `tukuyomi_proxy` | `conf/config.json` の `proxy.engine.mode` を一時的に書き換える。互換比較の時だけ `net_http` を指定する |
...
| `OUTPUT_FILE` | `data/logs/proxy/proxy-benchmark-summary.md` | 人間向け Markdown summary の出力先 |
| `OUTPUT_JSON_FILE` | `data/logs/proxy/proxy-benchmark-summary.json` | 機械可読 JSON の出力先 |
...
4. `BENCH_PROXY_ENGINE` は明示し、`net_http` と `tukuyomi_proxy` は別 run として比較する
