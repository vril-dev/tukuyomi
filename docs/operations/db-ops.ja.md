...
- `hot_log_retention_days`: DB 内に hot データとして残す WAF イベントの保持期間
...
`storage.log_archive.enabled=true` の場合、WAF log archive も
`persistent_storage` 配下に保存されます。
...
`storage.log_archive.enabled=true`（既定）の場合、期限を過ぎた完了済み UTC
日は、DB row を削除する前に gzip NDJSON archive として書き出します。既定の
保存先は次の形です。
...
`persistent_storage.backend=s3` の場合も、`persistent_storage.s3.prefix`
配下で同じ key layout を使います。`make install` は
`tukuyomi-waf-log-archive` という Scheduled Task を一度だけ bootstrap し、
既定では `03:17 UTC` に実行します。オペレーターが後でこの task を無効化、
変更、または削除した場合、以後の install では再作成しません。

`storage.log_archive.enabled=false` の場合は、従来どおり DB row を直接削除
します。
