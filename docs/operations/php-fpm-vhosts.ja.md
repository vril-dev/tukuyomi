...
- `/options`
  ...
  - 生成状態とプロセス状態の確認
- `/runtime-apps`
  - 管理対象 Runtime App 定義の管理元
  - ランタイムの待ち受けホスト、FastCGI 待ち受けポート、ドキュメントルート、使用ランタイム、rewrite、アクセスルール、Basic 認証、PHP ini 上書き設定の管理
  - 監視対象のローカル常駐プロセス向けに、app root、command、引数、実行ユーザー、restart policy、environment を管理
  - web runtime app 向け生成バックエンドの管理
- `/proxy-rules`
...
管理対象のアプリケーション定義は `/runtime-apps` で管理します。

`/runtime-apps` は、PHP-FPM / PSGI のビルド済みランタイムが 1 つ以上検出されている場合、または daemon Runtime App が設定されている場合に表示されます。

PHP-FPM Runtime App の必須項目は次のとおりです。
...
`request_slowlog_timeout` を設定すると、Xdebug なしで PHP-FPM slowlog を有効化
できます。`slowlog` path は tukuyomi が
...
`daemon` mode は、Gateway が配備と監視を担当するローカル常駐プロセス向けです。IoT broker process、adapter、ベンダー製制御 daemon のようなアプリケーションを想定します。

daemon Runtime App の必須項目は次のとおりです。
...
Center Runtime App Deploy では、daemon package は次の場所へ配備されます。

```text
data/app-deployments/<app-id>/current/app
```

`persistent_paths` は、release の洗い替え後も残すディレクトリです。Gateway は次の場所に実体を作成します。
...
daemon supervisor の出力先は次のファイルです。

```text
data/daemon-apps/<app-id>/daemon-supervisor.log
```

Gateway はこのログが無制限に増えないようにローテーションします。閉じたアーカイブは
圧縮済みの `*.log.gz` としてローカルに保持します。Center と連携していない Gateway
では、ローカルのアーカイブ保持だけを行います。Center に承認済みの Gateway では、
次回の署名付きステータスポーリングでローテーション済みアーカイブを対象デバイスの
`Runtime App Deploy` 画面へアップロードします。運用者は Center からアーカイブを
ダウンロードして調査に使い、保存期間を満たしたものは削除できます。書き込み中の
`daemon-supervisor.log` 本体はアップロードしません。
...
- slow request logging が有効な場合、生成された slowlog path も runtime state から確認できる
...
daemon Runtime App は `name` 単位で監視されます。`/runtime-apps` から `Start`、`Restart`、`Stop` を実行できます。App Deploy の restart behavior から、package 切り替え後に daemon を reload / restart することもできます。
