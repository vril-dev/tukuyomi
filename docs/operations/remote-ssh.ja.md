...
- Center と CLI は、Remote SSH で既定では HTTPS を要求します。HTTP を使えるのは、同一ホストの center-protected install で使う `127.0.0.1` や `::1` のような loopback IP 直指定の Center URL、またはローカル検証用の明示的なオプションを指定した場合だけです。
...
同一ホストの Center-protected Gateway を初期設定する場合、インストーラーは通常
`http://127.0.0.1:9092` のような loopback の Center URL を使います。Remote SSH は、
この内部 Gateway-to-Center 経路に限って loopback HTTP を許可します。外部の Center URL
へ接続する場合は HTTPS のまま運用し、必要に応じて CA / server name を設定してください。

同じ信頼設定は Gateway 設定へ書き込めます。
