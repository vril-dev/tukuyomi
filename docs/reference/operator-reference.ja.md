...
| `/proxy-rules` | vhost 以外の direct upstream / backend pool / route 編集と validate/probe/dry-run/apply/rollback |
| `/backends` | direct upstream backend object の一覧。direct named upstream は runtime enable / drain / disable / weight override に対応し、vhost generated target は Vhosts 側で扱う |
| `/sites` | site ownership と TLS binding |
| `/options` | runtime inventory, optional artifact, GeoIP/Country DB 管理 |
| `/vhosts` | static / `php-fpm` vhost の host、docroot、runtime、generated route 管理 |
...
- 選択順:
  1. explicit `routes[]`
  2. generated vhost host route
  3. DB `sites` domain 由来の generated host fallback route
  4. `default_route`
  5. `upstreams[]`
- host match は exact と `*.example.com`
- path match は `exact`, `prefix`, `regex`
- `upstreams[]` は vhost が所有しない direct backend node catalog です。各行は static `url` か `discovery` のどちらか一方を使います
- `backend_pools[]` は named upstream member から route 単位の balancing set を作る
- `action.backend_pool` は balancing の標準 route binding
- `action.upstream` は direct upstream 名または server-generated vhost upstream 名を参照できます。ただし通常の vhost traffic は `/vhosts` の generated host route で公開します
...
- `Vhosts` は effective runtime に generated host route と generated vhost backend を公開します。
- `Vhosts` は configured upstream URL を書き換えません。configured `primary` は `Proxy Rules > Upstreams` に表示される URL のままです。
- `generated_target` は server-owned の vhost materialization state で、通常の operator input ではありません。
- PHP-FPM/static app の接続先は `Proxy Rules > Upstreams` ではなく `Vhosts` で管理します。
...
DB `app_config` の `waf.engine.mode` で process-wide WAF engine を表します。
現在の build で稼働可能なのは Coraza のみですが、engine catalog は明示しており、
将来の adapter を登録できる前提にしています。
...
- `coraza` はこの build で利用可能な唯一の engine です。
- `mod_security` は将来用の既知 engine mode ですが、ModSecurity adapter が compile されていない build では利用不可です。config validation は fail-closed で拒否します。
- 未知の `waf.engine.mode` は config validation で拒否します。
- runtime visibility は `/tukuyomi-api/status` の `waf_engine_mode` / `waf_engine_modes` と `Settings -> Runtime Inventory` で確認できます。
- 左 navigation は `Security > Coraza` を engine-specific として扱い、active WAF engine が Coraza ではない場合は隠します。`Security > Request Controls` は Tukuyomi request policy なので表示を維持します。
- `Bypass Rules` は request control です。ただし `extra_rule` 参照は Coraza-backed DB-managed `.conf` snippet なので、active WAF engine が Coraza ではない場合は利用不可です。Full bypass entry は引き続き適用されます。

### Runtime Backend Operations

- normalized `upstream_runtime` DB domain が direct upstream / DNS discovery から materialize された backend key ごとの opt-in runtime override を保持します。`data/conf/upstream-runtime.json` は空 DB の seed/export path です
- `Backends` は direct upstream backend object を一覧しつつ、次の runtime 操作盤です
  - `enabled`
  - `draining`
  - `disabled`
  - 正の `weight_override`
...
- vhost generated target は `Vhosts` 側で扱い、`Backends` には表示しません
- route 直書き URL と vhost generated target は runtime 操作対象外です
...
managed な `extra_rule` body は DB `override_rules` に保存し、`Rules` > Advanced > `Bypass snippets` で編集します。`conf/rules` filesystem fallback はありません。起動時の base WAF rule set には混ぜず、bypass entry から logical `extra_rule` name で参照された時だけ load します。
...
- base WAF rule asset は disable できます。disable された asset は編集可能なまま、
  live WAF load set から外れます
- `/rules` は logical `rules/crs/rules/*.conf` name の DB-backed CRS asset を toggle
