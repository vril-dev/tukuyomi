...
managed な `extra_rule` body は DB `override_rules` に保存し、`Override Rules` から編集します。`conf/rules/*.conf` は新規DB import用のseed専用です。起動時の base WAF rule set には混ぜず、bypass entry から logical `extra_rule` path で参照された時だけ load します。
