...
| `/backends` | canonical backend object の一覧。direct named upstream は runtime enable / drain / disable / weight override に対応し、vhost generated backend は status-only |
...
- vhost generated backend は `Backends` に status-only の canonical object として表示されます
- route 直書き URL と vhost generated backend は runtime 操作対象外です
