...
managed な `extra_rule` body は DB `override_rules` に保存し、`Override Rules` から編集します。`conf/rules` filesystem fallback はありません。起動時の base WAF rule set には混ぜず、bypass entry から logical `extra_rule` name で参照された時だけ load します。
