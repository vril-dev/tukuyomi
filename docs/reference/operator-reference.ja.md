...
| `/backends` | direct upstream backend object の一覧。direct named upstream は runtime enable / drain / disable / weight override に対応し、Runtime App generated target は Runtime Apps 側で扱う |
| `/sites` | site ownership と TLS binding |
| `/options` | runtime inventory, optional artifact, GeoIP/Country DB 管理 |
| `/runtime-apps` | static / `php-fpm` / `psgi` application の runtime listener、docroot、runtime、generated backend 管理 |
...
- PHP runtime / Runtime Apps: [../operations/php-fpm-vhosts.ja.md](../operations/php-fpm-vhosts.ja.md)
...
- `upstreams[]` は Runtime Apps が所有しない direct backend node catalog です。各行は static `url` か `discovery` のどちらか一方を使います
- `backend_pools[]` は named upstream member から route 単位の balancing set を作る
- `action.backend_pool` は balancing の標準 route binding
- `action.upstream` は direct upstream 名または server-generated Runtime App upstream 名を参照できます
...
- `Runtime Apps` は effective runtime に runtime listener 用の generated backend を公開します。
- `Runtime Apps` は configured upstream URL を書き換えません。configured `primary` は `Proxy Rules > Upstreams` に表示される URL のままです。
- `generated_target` は server-owned の Runtime App materialization state で、通常の operator input ではありません。
- PHP-FPM/PSGI/static app の listener 設定は `Runtime Apps`、公開 traffic の routing は `Proxy Rules` で管理します。
...
- Runtime App generated target は `Runtime Apps` 側で扱い、`Backends` には表示しません
- route 直書き URL と Runtime App generated target は runtime 操作対象外です
