...
managed な `extra_rule` file は `conf/rules/*.conf` 配下に置き、`Override Rules` から編集します。これは起動時の base WAF rule set には混ぜず、bypass entry から参照された時だけ load されます。
`conf/rules/search-endpoint.conf` を参照する同梱 sample は `conf/waf-bypass.sample.json` を参照してください。
...
- `/rules` は active base rule file を編集
- `/rule-sets` は `rules/crs/rules/*.conf` を toggle
...
cache config は `data/conf/cache-rules.json`、internal cache store 設定は
`data/conf/cache-store.json` にあります。
