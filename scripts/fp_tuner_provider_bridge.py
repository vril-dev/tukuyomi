...
def build_host_scope(host: str, scheme: str) -> tuple[str, str]:
    host_only, port = split_host_port(host)
    if scheme == "http" and host_only and (not port or port == "80"):
        return "rx", "^" + re.escape(host_only) + "(:80)?$"
    if scheme == "https" and host_only and (not port or port == "443"):
        return "rx", "^" + re.escape(host_only) + "(:443)?$"
    return "streq", host


def build_rule_line(host: str, scheme: str, path: str, rule_id: int, variable: str) -> str:
    host_operator, host_operand = build_host_scope(host, scheme)
    digest = hashlib.sha1(f"{rule_id}|{host_operator}|{host_operand}|{path}|{variable}".encode("utf-8")).hexdigest()
    generated_id = 190000 + (int(digest[:6], 16) % 9000)
    return (
        f'SecRule REQUEST_HEADERS:Host "@{host_operator} {host_operand}" "id:{generated_id},phase:1,pass,nolog,chain,msg:\'tukuyomi fp_tuner scoped exclusion\'"\n'
        f'SecRule REQUEST_URI "@beginsWith {path}" "ctl:ruleRemoveTargetById={rule_id};{variable}"'
    )
...
def fill_proposal_defaults(proposal: Dict[str, Any], req_obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not str(out.get("summary", "")).strip():
        out["summary"] = f"Exclude {variable} from rule {rule_id} only under host {host} and path prefix {path}."
