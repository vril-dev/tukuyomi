...
test("computeSettingsRuntimeDrift returns empty when runtime matches configured values", () => {
  const config = {
    ...
    storage: { backend: "file", db_driver: "sqlite", db_path: "logs/coraza/tukuyomi.db" },
  ...
  const runtime = {
    ...
    storage_db_path: "logs/coraza/tukuyomi.db",
...
test("computeSettingsRuntimeDrift returns labeled mismatches when restart is pending", () => {
  const config = {
    ...
    storage: { backend: "db", db_driver: "mysql", db_path: "logs/coraza/custom.db" },
