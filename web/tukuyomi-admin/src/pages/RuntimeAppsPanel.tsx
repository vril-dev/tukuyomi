...
import {
  ...
  MAX_RUNTIME_APP_MAX_REQUEST_BODY_BYTES,
  comparableRuntimeAppRaw,
  createEmptyAccessRule,
  createEmptyRewriteRule,
  createEmptyRuntimeApp,
  parseRuntimeAppsResponse,
  runtimeAppsToRaw,
  validateRuntimeAppsForUI,
  type RewriteFlag,
  type RuntimeAppEntry,
  type RuntimeAppFormState,
  type RuntimeAppMode,
} from "@/lib/runtimeAppsEditor";
...
const BLOCKED_PHP_FPM_POOL_SETTINGS = [
  "listen",
  "listen.*",
  "user",
  "group",
                        ...
                        {app.mode === "php-fpm" ? (
                          <div className="space-y-1 text-xs md:col-span-2">
                            <div className="relative flex items-center gap-2">
                              <label
                                htmlFor={`php-pool-settings-${app.formID}`}
                                className="text-xs text-neutral-600"
                              >
                                {tx("PHP-FPM pool settings")}
                              </label>
                              <details className="relative">
                                <summary
                                  ...
                                  className="flex h-5 w-5 cursor-pointer list-none items-center justify-center rounded-full border border-neutral-300 bg-white text-[11px] font-semibold text-neutral-700 hover:bg-neutral-50"
                                >
                                  ?
                                </summary>
                                <div className="absolute left-0 top-6 z-20 w-[min(28rem,calc(100vw-3rem))] rounded border border-neutral-200 bg-white p-3 shadow-lg">
                                  <div className="mb-2 text-xs font-semibold text-neutral-700">
                                    {tx("Not configurable here")}
                                  </div>
                                  <div className="flex flex-wrap gap-1">
                                    {BLOCKED_PHP_FPM_POOL_SETTINGS.map(
                                      (name) => (
                                        <code
                                          key={name}
                            ...
                            <textarea
                              id={`php-pool-settings-${app.formID}`}
                              rows={5}
                              value={app.phpPoolSettingsText}
                              onChange={(e) =>
                                updateRuntimeApp(index, {
                                  ...app,
                                  phpPoolSettingsText: e.target.value,
                                })
                              }
                              className="w-full rounded border border-neutral-200 px-3 py-2 bg-white font-mono text-[11px]"
                              placeholder={
...
function RuntimeStateSummary({
  ...
  const slowLogFiles = process?.slowlog_files?.length
    ? process.slowlog_files
    : (materialized?.slowlog_files ?? []);
  ...
  return (
    <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 text-xs space-y-2">
      ...
      <div className="grid gap-1 text-xs text-neutral-600">
        ...
        {slowLogFiles.length > 0 ? (
          <div>
            {tx("Slow logs")}:{" "}
            <code className="break-all">{slowLogFiles.join(", ")}</code>
          </div>
        ) : null}
        {process?.last_error ? (
